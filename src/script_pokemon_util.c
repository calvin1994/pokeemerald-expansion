...
u32 BirchCase_GiveMonParameterized(u16 species, u8 level, u16 item, u8 ball, u8 nature, u8 abilityNum, u8 gender, u8 *evs, u16 *moves, bool8 ggMaxFactor, u8 teraType, bool8 isShinyExpansion)
{
    //
...
if (nature == 0xFF) // Zufalls-Nature
{
    nature = (Random() % 12) + 1; // wählt 1..12
}
    ...
    // EV and IV (IVs are excluded to be random)
    ...
    // ability
if (abilityNum == 0xFF) // Zufallsfähigkeit
{
    do {
        abilityNum = Random() % 3; // wählt 0, 1 oder 2

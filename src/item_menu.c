...
void UNUSED ItemMenu_Register(u8 taskId)
{
    ...
    BagMenu_PrintCursor(tListTaskId, COLORID_GRAY_CURSOR);
    tListPosition = listPosition;
    tQuantity = gBagPockets[gBagPosition.pocket].itemSlots[listPosition].quantity;
    gSpecialVar_ItemId = gBagPockets[gBagPosition.pocket].itemSlots[listPosition].itemId;
    sContextMenuFuncs[gBagPosition.location](taskId);
}
...
static void UNUSED ItemMenu_Deselect(u8 taskId)
{
    ...
    struct ItemSlot slot = GetBagItemIdAndQuantity(gBagPosition.pocket, listPosition);
